...
data = json.load(open('yidian_news.json', 'r', encoding='utf-8'))
parser = argparse.ArgumentParser()
parser.add_argument('-censor_words', required=True, help='list of censor words to remove from data')
opt = parser.parse_args()

if os.path.exists('yidian_news_clean.json'):
    clean_data = json.load(open('yidian_news_clean.json', 'r', encoding='utf-8'))
else:
    clean_data = {}
...
for id, content in data.items():
    ...
    cnt = len(clean_data)
    if cnt % 50 == 0:
        json.dump(clean_data, open('yidian_news_clean.json', 'w', encoding='utf-8'), indent=4, ensure_ascii=False)
        print('-'*50)
